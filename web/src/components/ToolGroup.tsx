  ...
  return (
    <Collapsible.Root defaultOpen={false}>
      <div className="border border-bdr rounded-lg overflow-hidden">
        <Collapsible.Trigger
          className="flex items-center gap-2 w-full px-3 py-2 bg-surface text-tx-2 text-[13px] font-sans cursor-pointer select-none text-left hover:bg-hovr transition-colors group"
        >
          <svg
        ...
        <Collapsible.Panel className="border-t border-bdr h-[var(--collapsible-panel-height)] overflow-hidden transition-[height] duration-200 data-[starting-style]:h-0 data-[ending-style]:h-0">
          {tools.map((tool, i) => (
            <ToolDetail key={i} tool={tool} isLast={i === tools.length - 1} />
          ))}
        </Collapsible.Panel>
      </div>
    </Collapsible.Root>
  ...
  return (
    <Collapsible.Root defaultOpen={false}>
      <div className={isLast ? "" : "border-b border-bdr"}>
        <Collapsible.Trigger
          className="flex items-center gap-2 w-full py-[7px] pl-7 pr-3 bg-elevated text-tx-2 text-xs font-mono cursor-pointer select-none text-left hover:bg-hovr transition-colors group"
        >
          <svg
